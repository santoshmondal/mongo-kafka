...
function clean_up {
    ...
    docker-compose exec mongo7 /usr/bin/mongo --eval "db.dropDatabase()"
    docker-compose down
    if [ -z "$1" ]
    then
      echo -e "Bye!\n"
...
curl -X POST -H "Content-Type: application/json" --data '
  {"name": "mongo-sink",
   "config": {
     ...
     "tasks.max":"1",
     "topics":"pageviews",
     "connection.uri":"mongodb://mongo7:27017",
     "database":"test",
...
docker-compose exec mongo7 /usr/bin/mongo --eval 'db.pageviews.find()'
...
Examine the collections:
  - In your shell run: docker-compose exec mongo7 /usr/bin/mongo
==============================================================================================================
